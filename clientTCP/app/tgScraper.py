...
class tgScraper(tgFetch):
    ...
    def getRelevantClasses(message, relevant=opts):
        ...
        classes = list(
            map(
                lambda x: list(
                    filter(
                        lambda y: y in relevant,
                        x["class"] if x.has_attr("class") else [],
                    )
                ),
                tags,
            )
    ...
    def dictFromMessage(msg, channel="", translator=None):
        ...
        try:
            ...
            print(f"(Message Views): {e}")
        try:
            dict["data"] = tgScraper.getMessageDate(msg)
        except Exception as e:
            print(f"(Message Date): {e}")
    ...
    def hasID(container):
        try:
            data_post = container.find("div", {"class": "tgme_widget_message"})[
                "data-post"
            ]
    ...
    def containers2dicts(containers, name="", min_id=1, translator=None):
        ...
        for container in containers:
            if (
                tgScraper.hasID(container)
                and int(tgScraper.extractID(container)) >= min_id
            ):
                dicts.append(
                    tgScraper.dictFromMessage(
                        container, channel=name, translator=translator
                    )
                )
        return dicts
    ...
    def getLastMessages(
        self, toFile=None, stdout=False, sendTCP=False, batch=True, sleepTime=25
    ):
        last_id = 0
        self.response = ""
        dicts = []
    ...
    def countMessagesInRequest(
        self, type="http", address=None, saveTo=None, source=None
    ):
        if type == "string":
            request = source
            address = ""
    ...
    def getAllMessages(
        ...
        while True:
            ...
            if self.countMessagesInRequest(
                type="string", source=html
            ) > 0 and current_id < max(tgScraper.getIdList(html)):
                current_id = tgScraper.getMaxId(html)
                parameters_list[len(parameters_list) - 1][1] = current_id
                new_dicts = tgScraper.html2dicts(
                    html, self.chname, translator=self.translator
                )
                self.sendTo(dict=new_dicts, stdout=stdout, sendTCP=sendTCP)
                tgScraper.saveData("last_id", current_id)
                dicts += new_dicts
